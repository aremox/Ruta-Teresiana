...
//Rare exceptions to indicate a bug in the _rutateresiana code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_rutateresiana = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_rutateresiana]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.rutateresiana.module"
